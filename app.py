...
from screen_agent_tools import screen_agent_tools, agent_launch_app, agent_disable_continuous_listening, agent_set_tts_enabled, agent_close_app, agent_open_app, agent_close_other_app, cancel_pending_screen_tools, agent_log_health_data, agent_press_back, agent_get_ui, agent_peek_app, agent_click, agent_insert_text, _send_tool_and_wait
...
TOOL_REGISTRY = {
    ...
    "agent_log_health_data": {
        "function_name": "agent_log_health_data",
        "requires_auth": False,
        "is_async": True,
        "needs_websocket": True,
        "args_mapping": lambda args, user_id, **kwargs: (
        ...
        "validation": lambda args: (
            {"error": "data_type is required."} if not args.get('data_type')
            else {"error": "data_type must be 'calories' or 'weight'."} if args.get('data_type') not in ("calories", "weight")
            else {"error": "value is required."} if args.get('value') is None
            else None
        ),
    },
