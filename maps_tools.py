...
maps_tools = [
    ...
    },
    {
        ...
        "description": "Get directions to a location in Google Maps. This tool automatically opens Google Maps. Use 'search' to find a location and get directions in one step (e.g., search='Slice House' or search='1885 Mission St'). You do not need an exact address; phrases like 'home', 'office', 'closest hardware store' are fine. If the user asks for directions to 'home', 'work', or 'office', pass that word DIRECTLY as the search query and let Google Maps resolve it. Do NOT check for or require a saved address, and do NOT ask the user for their address - even if no home address is saved, Google Maps will handle 'home'/'work' itself (it resolves them from the user's Google Maps saved places, or prompts in-app if not set). Never refuse or block a 'home'/'work' request for lack of a saved address. If already on a search results list, use 'position' or 'fragment' to select from it. If already on a location details page, no parameters needed. If you do not know the mode of transportation, you must call this tool without the mode specified so it can use the user's default mode of transportation. If you want to change the mode of transportation and are ALREADY on navigation, run this without the search parameter so it uses the location you already have.",
        "input_schema": {
            "type": "object",
            "properties": {
                "mode": {
