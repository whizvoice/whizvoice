...
def format_trigger_data(dump: dict) -> str:
    """Build the 'Trigger Data' section that points back to the Supabase row."""
    device_parts = [
        dump.get("device_manufacturer"),
        dump.get("device_model"),
    ]
    device = " ".join(p for p in device_parts if p) or "?"
    android = dump.get("android_version") or "?"
    w, h = dump.get("screen_width"), dump.get("screen_height")
    screen = f"{w}x{h}" if w and h else "?"

    recent = dump.get("recent_actions") or []
    if recent:
    ...
    return (
        f"## Trigger Data\n\n"
        f"- **Supabase row**: `screen_agent_ui_dumps.id = {dump['id']}`\n"
        f"- **Captured at**: {dump.get('created_at') or '?'}\n"
        f"- **App version**: {dump.get('app_version') or '?'}\n"
        f"- **Target package**: `{dump.get('package_name') or '?'}`\n"
        f"- **Device**: {device} (Android {android}, {screen})\n"
        f"- **Recent actions** (most recent first):\n{recent_block}\n\n"
        f"Fetch the full UI hierarchy + screenshot:\n"
        f"```\n"
        f"./get_recent_ui_dumps.py --id {dump['id']} --full --save\n"
        f"```\n"
    )
...
def create_pr(
    ...
    error_message = dump.get("error_message")
